...
| KPICard | `1197:1652` (`1102:6521`-`23`) | `packages/ui/src/primitives/KPICard.tsx` | `Primitives/KPICard` | Partially Synced — label/value typography rounded to nearest existing type-scale tier; success-tint bg color observed as a near-but-not-exact match, not re-verified; React only, WC/Angular deferred | 2026-07-20 |
| Theme Toggle | `1197:1652` (Header instance) | `packages/ui/src/primitives/ThemeToggle.tsx` | `Primitives/ThemeToggle` | Partially Synced — only Light-theme/idle instance sourced, no Dark-theme or mid-interaction evidence; track width rounded 54px→56px so thumb travel lands on a real spacing token; React only, WC/Angular deferred | 2026-07-20 |
| Page Header | `1197:1652` (`1102:6519`) | `packages/ui/src/composite/PageHeader.tsx` | `Composite/PageHeader` | Partially Synced — breadcrumb/description typography rounded to nearest existing tiers; breadcrumbs render inline rather than composing the separate Breadcrumb spec (§22 of component-specifications.md); React only | 2026-07-20 |
| Footer | `1197:1652` (`1102:6529`) | `packages/ui/src/layout/Footer.tsx` | `Layout/Footer` | Synced — React only, no cross-framework equivalent expected (layout-level) | 2026-07-20 |
| AppShell (`rail` variant) | `1197:1652` (NavigationRail instance) | `packages/ui/src/layout/AppShell.tsx` | `Layout/AppShell` (`Rail` story) | Partially Synced — only closed/collapsed rail sourced, no expanded-rail or open/close transition instance; additive alongside unchanged default `sidebar` variant | 2026-07-20 |
