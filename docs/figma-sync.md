...
| Button                     | `1027:3733` (collection frame `1174:1349`)                                                                       | `packages/ui/src/primitives/Button.tsx`, `packages/web-components/src/button/lumen-button.ts`, `packages/angular/src/button/lumen-button.ts`                              | `Primitives/Button` (`FinalVariantCollection`)                                                              | Synced — all 56 variant/state/theme combinations re-verified; six variants use the 34px geometry, Link uses 8px inline/2px block padding, and Accent uses the published crimson light/dark roles. Storybook's theme toolbar switches the same generated token modes; the standalone Theme Toggle has no instance in this node and remains sourced from its AppShell nodes.                 | 2026-07-22   |
| Input                      | `1262:1181`                                                                                                      | `packages/ui/src/primitives/Input.tsx`                                                                                                                                    | `Primitives/Input` (`VariantCollection`)                                                                    | Partially Synced — updated lg typography, exact size-specific border weights, sm/md/lg geometry, Default/Hover/Focused/Error roles, and search anatomy are bound; dark aliases are provisional because Figma publishes no dark Input mode; React is the current reference implementation                                                                                                   | 2026-07-21   |
...
| AppShell (responsive)      | `1007:3700`; desktop `1127:4196`/`1127:4197`, tablet `1175:2521`/`1175:2522`, mobile `1175:2588`/`1175:2589`     | `packages/ui/src/layout/AppShell.tsx`                                                                                                                                     | `Layout/AppShell` (`DesktopLight`, `DesktopDark`, `TabletLight`, `TabletDark`, `MobileLight`, `MobileDark`) | Synced — all six variants were re-extracted and the complete published color contract is mapped: app/surface, text, tertiary/link, icons, breakpoint-aware avatar, navigation foreground/background/selection, borders, badges, status, primary/secondary/accent buttons, assistant, `brand/dark`, and Theme Toggle. The story registers the exact 1440×900, 768×1024, and 390×844 frames. | 2026-07-22   |
