...
| KPICard | `1197:1652` (`1102:6521`-`23`) | `packages/ui/src/primitives/KPICard.tsx`, `packages/web-components/src/kpi-card/lumen-kpi-card.ts`, `packages/angular/src/kpi-card/lumen-kpi-card.ts` | `Primitives/KPICard` (React only — WC/Angular not covered) | Partially Synced — label/value typography rounded to nearest existing type-scale tier; success-tint bg color observed as a near-but-not-exact match, not re-verified; React/WC/Angular parity complete | 2026-07-20 |
...
| Page Header | `1197:1652` (`1102:6519`) | `packages/ui/src/composite/PageHeader.tsx` | `Composite/PageHeader` | Partially Synced — breadcrumb/description typography rounded to nearest existing tiers; breadcrumbs render inline rather than composing the separate Breadcrumb spec (§22 of component-specifications.md); React only, no cross-framework equivalent expected (composite/page-level) | 2026-07-20 |
| Footer | `1197:1652` (`1102:6529`) | `packages/ui/src/layout/Footer.tsx`, `packages/web-components/src/footer/lumen-footer.ts`, `packages/angular/src/footer/lumen-footer.ts` | `Layout/Footer` (React only — WC/Angular not covered) | Synced — React/WC/Angular parity complete; Angular's projected links are unstyled by default (no non-deprecated `::slotted` equivalent), documented known gap | 2026-07-20 |
