...
/**
 * Adapted from shadcn/ui's Breadcrumb — internal to @lumen/ui. Changes:
 * - imports resolve via this repo's existing relative-import convention
 * - lucide-react's `ChevronRight`/`MoreHorizontal` replaced with Lumen's
 *   own generated `ChevronRightIcon`/`EllipsisIcon`
...
const BreadcrumbList = React.forwardRef<HTMLOListElement, React.ComponentPropsWithoutRef<"ol">>(
  ({ className, ...props }, ref) => (
    <ol
      ref={ref}
      className={cn("flex flex-wrap items-center gap-1.5 break-words text-body-sm text-muted-foreground sm:gap-2.5", className)}
      {...props}
    />
  )
)
